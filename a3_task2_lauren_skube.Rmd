...
It removed the 3rd part of the name (after the second dash).
...
```{r}
# Your code here:
sw_skin_sep <- starwars %>%  # Start from starwars 
  separate(col = skin_color, # separate the skin_color column
           into = c("skin_1" , "skin_2"), # into two columns called "skin_1" and "skin_2"
           sep = ",", # separate after the first comma
           extra = "merge", # For additional commas, merge all into second column
...
```{r}
# Your code here: 
mpg_model_class <- mpg %>% # Start from mpg
...
some_dates_converted <- mdy(some_dates) # Converts to date and stores as "some_dates_converted"
...
gasol_hookshots <- lakers %>% # Start with lakers
  filter(player == "Pau Gasol" , type == "hook") %>% # only include observations for hook shots made by Pau Gasol
  mutate(date_new = ymd(date)) %>% # update the date column in a new column called "date_new"
  mutate(month = month(date_new)) # Add a new column called "month" that contains the month

hooks_by_month <- gasol_hookshots %>% # create a new subset from gasol_hookshots 
 count(month) # Find the number of hook shots recorded by month
...
storms_danger <- storms %>% # Start from storms
  mutate(danger = case_when( # add a new column called "danger" which contains...
  status == "hurricane" ~ "high", # "high" if status is hurricane...
  TRUE ~ "moderate" # "moderate" if status is anything else
  ))
